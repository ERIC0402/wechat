...
[@b.form action="!save" title="微信关键字管理" theme="list"]
	...
	[@b.textarea id="content" label="描述" name="wechatNewsItem.description" value="${(wechatNewsItem.description)!}" style="width:200px;" maxlength="200"/]
...
<script>
	...
	function addOreditTr(nameBefore){
		...
		if(nameBefore){
			parent.editTrStr($name("wechatNewsItem.title").val(), $name("wechatNewsItem.imagePath").val(), $name("wechatNewsItem.description").val(), $name("wechatNewsItem.url").val(), $name("wechatNewsItem.orders").val(), ${(wechatNewsItem.id)!'null'}, nameBefore);
		}else{
			parent.addTrStr($name("wechatNewsItem.title").val(), $name("wechatNewsItem.imagePath").val(), $name("wechatNewsItem.description").val(), $name("wechatNewsItem.url").val(), $name("wechatNewsItem.orders").val(), ${(wechatNewsItem.id)!'null'});
		}
	}
